...
import { useVsTypeStore } from "@/_store/newPost/vs"
...
const VsResultImage = ({ direction, ratio }: { direction: "left" | "right"; ratio: string }) => {
  const { leftCandidate, rightCandidate } = useVsTypeStore()
  const candidate = direction === "left" ? leftCandidate : rightCandidate

  console.log(ratio)

  return (
    <div className="vs-candidate vs-result">
...
const Dropzone = ({ direction }: { direction: "left" | "right" }) => {
  const { leftCandidate, rightCandidate, setCandidate } = useVsTypeStore()
  const candidate = direction === "left" ? leftCandidate : rightCandidate
...
export default function VsTypeContent() {
  const { section } = useNewPostStore()
  const { leftCandidate, rightCandidate, setCandidate } = useVsTypeStore()
  const isResultPage = section === "result"
  ...
  return (
    <div className={classNames("content", "vs-type")}>
      {(["left", "right"] as Array<"left" | "right">).map((dr) => (
        <div key={dr} className={dr}>
          {isResultPage ? (
            <VsResultImage ratio={dr === "left" ? ratio.left : ratio.right} direction={dr} />
          ) : (
